...
- 关键依赖：`pinchtab@0.7.7`（浏览器自动化）、`node:sqlite + sqlite-vec`（记忆存储）
...
2. **docs/plan/**：Plan / Research / Report 文档
3. **docs/tasks/**：历史任务归档，不再承接现役任务
4. **docs/CHANGELOG.md**：外部可见变更日志

### 9.3 提交规范
- PR 标题引用 issue：`Issue: NNNN`
- 若有 Plan 文档必须引用：`Plan: docs/plan/plNNNN.<state>.<board>.<slug>.md`
