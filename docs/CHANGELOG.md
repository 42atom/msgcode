...
- 2026-03-12
  - runtime/transports: 正式消息主链已收口为 Feishu-only；`src/imsg/`、`vendor/imsg/` 与 legacy `imsg` 启动测试已迁入 `.trash/2026-03-12-imsg-sunset/`，`MSGCODE_TRANSPORTS=imsg` 只会在 bootstrapped 真实运行入口显式报 sunset 错误，启动必需依赖也固定收口为飞书凭据 (Issue: 0093, Plan: docs/design/plan-260312-feishu-only-channel-simplification-and-imsg-sunset.md) [risk: high] [rollback: 从 `.trash/2026-03-12-imsg-sunset/` 恢复 legacy 代码，并回退 `src/config{,.transports}.ts`、`src/deps/{load,manifest.json}`、`src/{index,daemon,commands}.ts` 与相关测试]
