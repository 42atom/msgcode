...
## 能力硬门（P5.7-R1c 冻结，后续任务必须引用）

### 新增命令必须满足（5 项硬门）

1. **命令语义 = 真实行为**（如 `send` 必须真发送，`fetch` 必须真抓取）
2. **至少 1 条真实成功证据**（非 mock、非静态断言，真实调用外部系统）
3. **至少 1 条真实失败证据**（真实错误码 + 错误消息，退出码非 0）
4. **`help-docs --json` 必须可发现**（命令合同完整，必填/可选参数明确）
5. **错误码必须固定枚举**（禁止仅文本提示，必须有结构化 `errorCode`）
...
- `exitCode`: `0` | `2` | `1`
- `data`: 业务数据（含 `ok`, `sendResult`/`fetchResult` 等）
- `errors`: Diagnostic[]（含 `code`, `message`, 可选 `hint`/`details`）
