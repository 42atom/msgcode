...
**events.ndjson 格式**：每行一个 JSON 对象（NDJSON）

**最小事件类型（P0）**：

1. **desktop.start**：方法开始执行
```jsonc
{
  "type": "desktop.start",
  ...
  "params": { ... }  // 可选，请求参数（不含敏感数据）
}
```

2. **desktop.stop**：方法执行结束
```jsonc
{
  "type": "desktop.stop",
  ...
  "executionId": "uuid",
  "method": "desktop.observe",
  "result": { "evidenceDir": "..." }  // 可选，返回摘要
}
```

3. **desktop.error**：方法执行出错
```jsonc
{
  "type": "desktop.error",
...
4. **desktop.observe**：observe 专用事件（T10）
```jsonc
{
  "type": "desktop.observe",
  "timestamp": "2026-02-10T12:00:00Z",
  "executionId": "uuid",
  "permissionsMissing": ["accessibility"],  // 可选，缺失的权限
  "screenshotPath": "observe.png",         // 可选，仅当成功时
  "axPath": "ax.json",                     // 可选，仅当成功时
  "envPath": "env.json"                    // 总是存在
}
```
...
```jsonc
{
  ...
  "eventsPath": "events.ndjson",  // T10: 新增
  "peer": { ... },
  "host": { ... },
  "permissions": { ... }
}
