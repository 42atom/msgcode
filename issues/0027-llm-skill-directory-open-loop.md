...
- [x] 创建 issue + plan
- [x] 查清哪些地方在阻止 LLM 继续
- [x] 收正式口径：向 LLM 注入 skills 目录与环境路径
- [x] 收底层工具面：开放必要工具
- [x] 收中途阻断：去掉不合理拦截
- [x] 测试（27 tests pass）
- [ ] 真机 smoke
...
- LLM 读取 skill 后可以继续使用必要底层工具
- 工具暴露不再被 `pi.enabled` 或消息级裁判错误拦住
- 至少一组测试锁住 open-loop 的工具暴露语义
- 真机 smoke 可作为收尾项单独验证，不阻断当前主链
...
- Plan: `docs/design/plan-260308-llm-skill-directory-open-loop.md`
- Issue: `issues/0026-agent-read-skill-bridge.md`
